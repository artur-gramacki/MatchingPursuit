...
and can also be generated by the EMPI program; see Section 14 for details.
...
## Pure R implementation of the MP algorithm
The package interfaces with the external EMPI program, a multi-threaded C++ implementation of the Matching Pursuit (MP) algorithm with GPU support and CPU parallelization. The implementation supports optimal dictionaries, including the simulation of continuous (quasi-infinite) dictionaries (switch `-o global`). By default, atoms are allowed to extend beyond the signal boundaries, with the signal assumed to be zero outside its observed range. This behavior can be changed using the `--full-atoms-in-signal` switch, which restricts atoms to lie entirely within the signal (see also Section 13 for details). 
...
```{r mp_pure_R_impl, include = TRUE, echo = TRUE, warning = FALSE, fig.width = 7, fig.height = 7}
out_mp <- mp_omp_run_pipeline(
   mode = 'mp',             # switch to MP algo
   sig_file = sig_file,
   col_names_in_csv = TRUE,
   xml_file = xml_file,
   topk = 5000,

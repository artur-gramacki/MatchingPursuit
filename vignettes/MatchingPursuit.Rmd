...
atom.params(file)
```
In the next example, note that the `EEG_bipolar_filtered.db` file contains info on 18 channels. To keep the output concise, only the first 5 and last 5 atoms are displayed below. The total number of atoms generated is 900 (number of atoms per channel = 50, number of channels = 18, so 50 × 18 = 900).  

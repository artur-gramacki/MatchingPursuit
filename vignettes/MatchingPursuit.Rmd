...
To display the T–F map, you can also use `plot.empi()`, the `S3` method for the generic `plot()` function. This function requires an object of class `empi`, created by `empi.execute()`. Due to CRAN regulations, the code below has been commented out. See the explanation [here](#important_note). Uncomment it to view the result.

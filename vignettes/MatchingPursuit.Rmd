...
```{r instalation, include = TRUE, echo = TRUE, warning = FALSE, message = FALSE}
# Install and load required R packages
pkgs <- c(
  "edf", "signal", "RSQLite", "DescTools", "imager", "raster", 
  "graphics", "grDevices", "utils", "knitr", "rmarkdown", "latex2exp", 
  "knitr", "rmarkdown", "remotes",  "digest", "signal")
...
The presented package enables the analysis of time-series signals using the Matching Pursuit (MP) algorithm (see @Mallat-1993, @Tropp-2008). Additionally, it supports working with EEG (electroencephalogram) signals by:
...
Let’s generate an example by mixing seven different non-stationary signals. The resulting signal (highlighted in blue below) will be used to demonstrate the basic functionality of the package.
...
The input data (`signal`) is passed as an argument to the `empi.execute()` function, which generates the final output file in `SQLite` format (`empi.db`) containing all atom parameters. This `.db` file is stored in a temporary directory created by the `tempfile()` function.

The function first downloads (if it has not been downloaded previously) and then runs an external program from the Internet. This program is a fast and efficient implementation of the Matching Pursuit algorithm. The author refers to it as **Enhanced Matching Pursuit Implementation**, or **empi** for short (see @Rozanski-2024 for details). The download link and resource name are displayed below. The appropriate binary version is downloaded depending on the operating system (Windows, Linux, or macOS).
...
**Important note:**
The code in the chunk below has been commented out because CRAN's verification rules prohibit automatic binary downloads. Therefore, this function should not be executed in this context. However, when the function is run in interactive mode (i.e., when `interactive()` returns `TRUE`), the user is prompted to confirm whether they agree to download the external binary resource. If they consent, the required files are downloaded and saved in the current directory. Due to CRAN limitations, the output of the `empi.execute()` function (the `sample1.db` file) has been generated in advance and included in the package.

In the next chunk, the `empi2ft()` function uses this file as input (the `db.file` parameter).

Notice the `empi.options` parameter in the `empi.execute()` function. You can specify `NULL` for this parameter, and the *empi* program will run with the default values set in the function (`"-o local --gabor -i 50"`). It is also worth noting that the program offers a wide range of configuration options. Details can be found in the `README.md` file located in the directory where the *empi* program was saved. In our example, parameters were set to instruct the program to find 25 atoms.
...
Generating the final time-frequency (T-F) map for the selected channel. The centers of the atoms (in terms of time and frequency coordinates) are marked with the numbers of successive atoms, sorted from highest to lowest energy.  
...
In this section, we demonstrate how to analyze electroencephalography (EEG) signals using the Matching Pursuit algorithm. The package includes a dedicated function for reading files saved in EDF and EDF+ formats (European Data Format). You can also perform three different types of montages and apply filtering to the signals.

### Step 1 {.unnumbered}

Reading an example EEG signal (EDF file). The signal is 10 seconds long and consists of 20 channels. The sampling rate is 256 Hz. The channels have standard names, which are displayed below.
...
EEG signals are rarely analyzed without prior filtering. Using the `filters.coeff()` function, you can define the filter parameters and then apply the filter to the signal. The filter parameters given below use typical values recommended in the literature.
...
(ap <- atom.params(file))
```
In this example, note that the `EEG_bipolar_filtered.db` file contains 18 channels. To keep the output concise, only the first 5 and last 5 atoms are displayed below. The total number of atoms generated is 900 (number of atoms per channel = 50, number of channels = 18, 50 × 18 = 900).  
...
The `sig2bin()` function can be used to convert input data (in plain text file format) to a binary file. This function is used internally by the `empi.execute()` function. The binary data consist of floating-point values in the byte order of the current machine (no byte-order conversion is performed). For multichannel signals, the samples are arranged such that first come all channels at `t = 0`, then all channels at `t = Δt`, and so on. In other words, the signal should be written in column-major order (rows = channels, columns = samples).

**Note:** Users do not work directly with `.bin` files. Binary files are used only by the `empi.execute()` function. The external **empi** program executed within this function requires binary data as input. Additionally, the ability to convert text files to binary form can be useful for those who wish to use **empi** independently of the R environment.
...
Data from the generated `SQLite` file can be easily read using the `read.empi.db.file()` function. Below, data from a sample `.db` file is read. The original and reconstructed signals are then extracted and displayed. The two signals are virtually indistinguishable, indicating that the atoms are highly effective at reconstructing the analyzed signal.
...
par(mfcol=c(8, 1), pty="m") #bottom, left, top, right
...
plot(t, g1$gabor, ylim = ylim, xaxt = "n", yaxt = "n", bty = "o", col = "brown", type = "l", xlab = "", ylab = "",   main = "Gabor: f = 50Hz, mean = 2, sigma = 1")
plot(t, g2$gabor, ylim = ylim, xaxt = "n", yaxt = "n", bty = "o", col = "brown", type = "l", xlab = "", ylab = "",   main = "Gabor: f = 25Hz, mean = 8, sigma = 1.5")
plot(t, g3$gabor, ylim = ylim, xaxt = "n", yaxt = "n", bty = "o", col = "brown", type = "l", xlab = "", ylab = "",   main = "Gabor: f = 30Hz, mean = 2, sigma = 2")
plot(t, g4$gabor, ylim = ylim, xaxt = "n", yaxt = "n", bty = "o", col = "brown", type = "l", xlab = "", ylab = "",   main = "Gabor: f = 15Hz, mean = 7, sigma = 0.5")

...
#')
#'
#' bip.mont <- eeg.montage(signal, montage.type = c("bipolar"), bipolar.pairs = pairs)
#' ref.mont <- eeg.montage(signal, montage.type = c("reference"), ref.channel = "O1")
#' avg.mont <- eeg.montage(signal, montage.type = c("average"))
#'
#' head(bip.mont)
#' head(ref.mont)
#' head(avg.mont)

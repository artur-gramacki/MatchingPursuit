...
#'
#' @param x An object of class \code{empi} or a path to a SQLite file created by \code{empi.execute()}.
#'
...
#' the maximum frequency displayed in the T-F map will be \code{ceiling(f / 2 / freq.divide)}
...
#'
#' @param shortening.factor.x Usually, a value of 2 provides better atom visualization.
#'
#' @param shortening.factor.y Usually, a value of 2 provides better atom visualization.
#'
...
#'
#' @param palette PPalette from the list returned by the \code{hcl.pals()} function or the string
...
#'      \item \code{"file"} - saves a T-F map to the file \code{file.name} (as a \code{png} file).
#'      \item \code{"RData"} - saves the T-F map of size \code{size} to \code{file.name}
#'      (as an R matrix); resampling is performed using the \code{imager::resize()} function.
#'      \item \code{"RData2"} - saves the T-F map of size \code{size} to \code{file.name}
...
#'
#' @param file.name Name of the \code{png} file (if \code{out.mode = "file"}) or name of the
#' \code{RData} file (if \code{out.mode = "RData"} or \code{out.mode = "RData2"}).
#'
#' @param size Size of the \code{png} file in pixels (if \code{out.mode = "file"}) or size of
#' the T-F matrix (if \code{out.mode = "RData"} or \code{out.mode = "RData2"}).
#'
#' @param draw.ellipses Intended for testing only. Can be set to \code{TRUE} to display
#' the effect. Works correctly only if \code{out.mode = "plot"}.
#'
#' @param plot.signals Whether the original and reconstructed signals should also be displayed.
#'
#' @param write.atoms If \code{TRUE}, writes all atom plots to the \code{Atoms.pdf} file
#' (in the cache directory or in a user-specified directory, depending on \code{path}).
#'
#' @return Depending on the \code{out.mode} parameter, the function:
#'    \itemize{
...
#'     (if \code{out.mode = "RData"} or \code{out.mode = "RData2"}; otherwise \code{NULL})
